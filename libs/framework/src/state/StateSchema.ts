...
export class StateSchema<T extends StateRecord = any> {
  ...
  private constructor(private readonly map: Map<keyof T, T[keyof T]>) {}
...
export type StateStorage<R extends StateSchema = StateSchema> =
  R extends StateSchema<infer T> ? StateRecordStorage<T> : never;

export type StateRuntime<R extends StateSchema = StateSchema> =
  R extends StateSchema<infer T> ? StateRecordRuntime<T> : never;
